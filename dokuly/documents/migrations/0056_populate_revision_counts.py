...
def populate_document_revision_counts(apps, schema_editor):
    """Populate revision_count_major and revision_count_minor for all documents."""
    Document = apps.get_model('documents', 'Document')
    
    updated_count = 0
    for document in Document.objects.all():
        if document.revision:
            major, minor = parse_revision_to_counts(document.revision)
            document.revision_count_major = major
            document.revision_count_minor = minor
            document.save(update_fields=['revision_count_major', 'revision_count_minor'])
            updated_count += 1
    
    print(f"Updated {updated_count} documents with revision counts")


def reverse_migration(apps, schema_editor):
    """Reset revision counts to 0."""
    Document = apps.get_model('documents', 'Document')
    Document.objects.all().update(revision_count_major=0, revision_count_minor=0)


class Migration(migrations.Migration):

    dependencies = [
        ('documents', '0055_document_revision_count_major_and_more'),
    ]

    operations = [
        migrations.RunPython(populate_document_revision_counts, reverse_migration),
    ]

...
def populate_pcba_revision_counts(apps, schema_editor):
    """Populate revision_count_major and revision_count_minor for all pcbas."""
    Pcba = apps.get_model('pcbas', 'Pcba')
    
    updated_count = 0
    for pcba in Pcba.objects.all():
        if pcba.revision:
            major, minor = parse_revision_to_counts(pcba.revision)
            pcba.revision_count_major = major
            pcba.revision_count_minor = minor
            pcba.save(update_fields=['revision_count_major', 'revision_count_minor'])
            updated_count += 1
    
    print(f"Updated {updated_count} pcbas with revision counts")


def reverse_migration(apps, schema_editor):
    """Reset revision counts to 0."""
    Pcba = apps.get_model('pcbas', 'Pcba')
    Pcba.objects.all().update(revision_count_major=0, revision_count_minor=0)


class Migration(migrations.Migration):

    dependencies = [
        ('pcbas', '0057_pcba_revision_count_major_pcba_revision_count_minor'),
    ]

    operations = [
        migrations.RunPython(populate_pcba_revision_counts, reverse_migration),
    ]

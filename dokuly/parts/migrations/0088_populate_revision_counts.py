...
def populate_part_revision_counts(apps, schema_editor):
    """Populate revision_count_major and revision_count_minor for all parts."""
    Part = apps.get_model('parts', 'Part')
    
    updated_count = 0
    for part in Part.objects.all():
        if part.revision:
            major, minor = parse_revision_to_counts(part.revision)
            part.revision_count_major = major
            part.revision_count_minor = minor
            part.save(update_fields=['revision_count_major', 'revision_count_minor'])
            updated_count += 1
    
    print(f"Updated {updated_count} parts with revision counts")


def reverse_migration(apps, schema_editor):
    """Reset revision counts to 0."""
    Part = apps.get_model('parts', 'Part')
    Part.objects.all().update(revision_count_major=0, revision_count_minor=0)


class Migration(migrations.Migration):

    dependencies = [
        ('parts', '0087_part_revision_count_major_part_revision_count_minor'),
    ]

    operations = [
        migrations.RunPython(populate_part_revision_counts, reverse_migration),
    ]

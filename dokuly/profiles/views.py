...
def manage_user_subscriptions(request):
    ...
    if request.method == 'PUT':
        if 'session_id' not in d:
            return Response("Cannot verify the checkout session.", status=status.HTTP_400_BAD_REQUEST)
        try:
            # Fetch the session
            # Fetch profile and org
            ...
            org_data = OrganizationManagerSerializer(org, many=False).data
            # redir after checkout
            redir_url = f"https://{request.tenant}.dokuly.com/#/adminPage"
            local_server = bool(int(os.environ.get('DJANGO_LOCAL_SERVER', 0)))
            if local_server:
                redir_url = f"http://{request.tenant}.dokuly.localhost:8000/#/adminPage"
            return Response(redir_url, status=status.HTTP_201_CREATED)
        except Exception as e:
            return Response(str(e), status=status.HTTP_400_BAD_REQUEST)
    if request.method == 'GET':
        try:
            ...
            # Serialize the Subscription objects
    ...
    if request.method == 'POST':
        try:
            with transaction.atomic():
                ...
                # Ensure 'id' and other needed fields are in request data
                d = request.data
                if 'id' not in d or d['id'] is None:
                    return Response("No valid sub id sent with the request", status=status.HTTP_400_BAD_REQUEST)

                # Fetch subscription object and determine the maximum allowed users
                org = get_object_or_404(
                    Organization, id=profile.organization_id)
                sub = get_object_or_404(Subscription, organization=org)
                allowed_users = sub.subscription_info.get(
                    'quantity', None)
                if allowed_users == None:
                    return Response("Allowed users is null", status=status.HTTP_400_BAD_REQUEST)
                if d['quantity'] > 0:
                    Organization.objects.filter(id=data['organization_id']).update(
                        max_allowed_active_users=allowed_users
                    )
                else:
                    return Response("Quantity input error", status=status.HTTP_400_BAD_REQUEST)
            # Check active / allowed
            # Deactivate a basic user if needed
            users = ProfileSerializer(Profile.objects.all(), many=True).data
            active_users = 0
            for user in users:
                if user.get('is_active') and user.get("role") != "Viewer":
                    active_users += 1
            if active_users > allowed_users:
                low_prio_users = Profile.objects.filter(role="User")
                if low_prio_users.exists():
                    low_prio_users_data = ProfileSerializer(
                        low_prio_users, many=True).data
                    Profile.objects.filter(id=low_prio_users_data[0]['id']).update(
                        is_active=False
                    )
                mid_prio_users = Profile.objects.filter(role="Developer")
                if mid_prio_users.exists():
                    mid_prio_users_data = ProfileSerializer(
                        mid_prio_users, many=True).data
                    Profile.objects.filter(id=mid_prio_users_data[0]['id']).update(
                        is_active=False
                    )
                high_prio_users = Profile.objects.filter(role="Admin")
                if high_prio_users.exists():
                    high_prio_users_data = ProfileSerializer(
                        high_prio_users, many=True).data
                    Profile.objects.filter(id=high_prio_users_data[0]['id']).update(
                        is_active=False
                    )
            return Response("Subscriptions Updated", status=status.HTTP_200_OK)
        except Exception as e:
            return Response(str(e), status=status.HTTP_400_BAD_REQUEST)
    return Response(f"{request.method} not allowed", status=status.HTTP_406_NOT_ACCEPTABLE)

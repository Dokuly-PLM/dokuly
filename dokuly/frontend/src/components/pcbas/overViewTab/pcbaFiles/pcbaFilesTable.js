  ...
  const columns = [
    ...
    },
    {
      ...
      formatter: (row) => {
        ...
        if (revisionLocked) {
          const category = CATEGORY_OPTIONS.find(opt => opt.value === row.file_category);
          return <span>{category?.label || "Design"}</span>;
        }
        return (
          <GenericDropdownSelector
            state={row.file_category || "design"}
            setState={(newValue) => handleSaveFileCategory(row.file_id || row.id, newValue)}
